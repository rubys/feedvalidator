...
__date__ = "$Date$"
_copyright__ = "Copyright (c) 2004 Joseph Walton"
__license__ = "Python"
...
_decUCS2BE = codecs.getdecoder('UCS-2BE')
_decUCS2LE = codecs.getdecoder('UCS-2LE')
_decEBCDIC = codecs.getdecoder('IBM037') # EBCDIC
_decACE = codecs.getdecoder('ISO-8859-1') # An ASCII-compatible encoding

# Note that only the encodings in 'permitted' will be accepted.
# This is so that, e.g., a 4-byte-character XML file that claims to be US-ASCII
#  will fail now.
def _decodeDeclaration(sig, dec, permitted):
  encoding = _encodingFromDecl(dec(sig)[0])
  if not(encoding):
    from exceptions import UnicodeError
    raise UnicodeError('This XML file requires an encoding declaration')
  if permitted and not(encoding.upper() in permitted):
    from exceptions import UnicodeError
    raise UnicodeError('This XML file claims an encoding of ' + encoding + ', but looks more like ' + permitted[0])
  return encoding
...
def detect(doc_start):
  """This is the logic from appendix F.1 of the XML 1.0 specification.
  Pass in the start of a document, and receive the encoding to use."""
  sig = doc_start[:4]
  ...
  # With a BOM. Use byte-order-neutral encoding
  #  names, so the BOMs get stripped by the codecs
  elif sig == '\x00\x00\xFE\xFF':  # UCS-4 BE
    enc = 'UCS-4'
  elif sig == '\xFF\xFE\x00\x00':  # UCS-4 LE
    enc = 'UCS-4'
  elif sig == '\x00\x00\xFF\xFE'  or sig == '\xFE\xFF\x00\x00':
    raise 'Unable to process UCS-4 with unusual octet ordering'
  ...
  # Without a BOM; we read the declaration
  elif sig == '\x00\x00\x00\x3C':
    enc = _decodeDeclaration(doc_start, _decUCS4BE, ['UCS-4BE', 'UTF-32BE'])
  elif sig == '\x3C\x00\x00\x00':
    enc = _decodeDeclaration(doc_start, _decUCS4LE, ['UCS-4LE', 'UTF-32BE'])
  elif sig == '\x00\x3C\x00\x3F':
    enc = _decodeDeclaration(doc_start, _decUCS2BE, ['UTF-16BE', 'UCS-2BE'])
  elif sig == '\x3C\x00\x3F\x00':
    enc = _decodeDeclaration(doc_start, _decUCS2LE, ['UTF-16LE', 'UCS-2LE'])
  elif sig == '\x3C\x3F\x78\x6D':
    enc = _decodeDeclaration(doc_start, _decACE, None)
  elif sig == '\x4C\x6F\xA7\x94':
    enc = _decodeDeclaration(doc_start, _decEBCDIC, ['IBM037', 'CP037', 'IBM038', 'EBCDIC-INT'])
  else:
    # It's UTF-8, or mislabelled
    enc = 'UTF-8'
