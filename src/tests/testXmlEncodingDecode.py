...
class TestDecode(unittest.TestCase):
  ...
  def testProvidedEncoding(self):
    loggedEvents=[]
    self._assertEqualUnicode(xmlEncoding.decode('UTF-8', '<x/>', loggedEvents), u'<x/>')
    self.assertEqual(loggedEvents, [])

    loggedEvents=[]
    self._assertEqualUnicode(xmlEncoding.decode('UTF-8', '<?xml version="1.0" encoding="utf-8"?><x/>', loggedEvents), u'<?xml version="1.0" encoding="utf-8"?><x/>')
    self.assertEquals(loggedEvents, [])

  def testNoDeclarationOrBOM(self):
    loggedEvents=[]
    self.assertEquals(xmlEncoding.decode(None, '<x/>', loggedEvents), None)
...
#    loggedEvents=[]
#    self._assertEqualUnicode(xmlEncoding.decode(None, '<?xml version="1.0"?><x/>', loggedEvents), u'<?xml version="1.0"?><x/>')
#    self.assertEquals(len(loggedEvents), 1)
#    self.assertEquals(loggedEvents[0].__class__, MissingEncoding, "Must warn if there's no clue as to encoding")

  def testJustDeclaration(self):
    loggedEvents=[]
    self._assertEqualUnicode(xmlEncoding.decode(None, '<?xml version="1.0" encoding="utf-8"?><x/>', loggedEvents), u'<?xml version="1.0" encoding="utf-8"?><x/>')
    self.assertEquals(loggedEvents, [])
  ...
  def testDeclareUnknownEncoding(self):
    loggedEvents=[]
    self.assertEquals(xmlEncoding.decode(None, '<?xml version="1.0" encoding="X-FAKE"?><x/>', loggedEvents), None)
    self.assert_(loggedEvents)
    self.assertEquals(loggedEvents[-1].__class__, UnknownEncoding)

  def testWarnMismatch(self):
    loggedEvents=[]
    self.assertEquals(xmlEncoding.decode('US-ASCII', '<?xml version="1.0" encoding="UTF-8"?><x/>', loggedEvents), u'<?xml version="1.0" encoding="UTF-8"?><x/>')
    self.assert_(loggedEvents)
    self.assertEquals(loggedEvents[-1].__class__, EncodingMismatch)

  def testDecodeUTF8(self):
    loggedEvents=[]
    self.assertEquals(xmlEncoding.decode('utf-8', '<x>\xc2\xa3</x>', loggedEvents), u'<x>\u00a3</x>')
    self.assertEquals(loggedEvents, [])

  def testDecodeBadUTF8(self):
    """Ensure bad UTF-8 is flagged as such, but still decoded."""
    loggedEvents=[]
    self.assertEquals(xmlEncoding.decode('utf-8', '<x>\xa3</x>', loggedEvents), u'<x>\ufffd</x>')
    self.assert_(loggedEvents)
  ...
  def testRemovedBOM(self):
    ...
    loggedEvents=[]
    self.assertEquals(xmlEncoding.decode('UTF-16', '\xff\xfe\x3c\x00\x78\x00\x2f\x00\x3e\x00', loggedEvents), u'<x/>')
    self.assertEquals(loggedEvents, [])
